...
class HistoricalModels(models.Model):
    ...
    history_related_key = models.CharField(default='', max_length=64, verbose_name='用于关联外键的变化', db_index=True)
    ...
    def get_history_change(self, history_old, history_fields=None):
        ...
        # 总会返回的字段
        history_labor_income_tax_mode = history_old.labor_income_tax_mode if hasattr(history_old,
                                                                                     'labor_income_tax_mode') else None
        for change in delta.changes:
            ...
            verbose_name = self.instance_type._meta.get_field(change.field).verbose_name.title()
            ...
            if hasattr(self.instance_type, 'oss_url_fields'):
                if change.field in self.instance_type.oss_url_fields:
                    if change_old:
                        change_old = get_shared_signature_full_path(full_path=change_old)
                    if change_new:
                        change_new = get_shared_signature_full_path(full_path=change_new)
    ...
    def set_history_change_reason(self):
        ...
        # 变更 ForeignKey 关联
        self.history_related_key = foreign_key_class_name + '_' + history_related_key if foreign_key_class_name else history_related_key
    ...
    def test_for_change(instance, validated_data):
        try:
            ...
            for fields in instance.History.history_fields:
                if '.' in fields and (validated_data.get(fields.split('.')[0]) and not isinstance(
                        validated_data.get(fields.split('.')[0]), dict)):
            ...
            validated_data_fields = list(set(validated_data_fields) - set(ignore_fields))
            is_history = False
            for field in validated_data_fields:
                if '__' in field:
                    field = field.replace('__', '.')
                if hasattr(instance, 'oss_url_fields') and field in instance.oss_url_fields:
                    ...
                    if getattr(related, field.split('.')[1]) != validated_data[field.split('.')[0]][
                        field.split('.')[1]]:
                        is_history = True
                        return is_history
